...
class WebsiteSale(WebsiteSale):
    ...
    def product(self, product, category='', search='', **kwargs):
        ...
        domain = expression.AND([
            ['&', ('state', 'in', ['enabled', 'test']), ('company_id', '=', request.website.company_id.id)],
            ['|', ('website_id', '=', False), ('website_id', '=', request.website.id)],
            [('provider', '=', 'cash_on_delivery')],
        ])
        cod = request.env['payment.acquirer'].sudo().search(domain, limit=1)
        
        is_cod_available = cod and request.website.is_cod_available(product)
        res.qcontext['cod_availability'] = is_cod_available 
        res.qcontext['cod_rule'] = cod and cod.cod_rule
        return res
