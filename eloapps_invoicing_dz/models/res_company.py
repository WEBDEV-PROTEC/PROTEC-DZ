...
class ResCompany(models.Model):
    ...
    activity_code_id    = fields.Many2many('elo.activity.code', compute='elo_compute_fiscal_information', inverse='elo_inverse_activity_code_id', string ="Code d'activité", index=True)
    forme_juridique_id  = fields.Many2one('elo.forme.juridique', compute='elo_compute_fiscal_information', inverse='elo_inverse_forme_juridique_id', string="Forme juridique")
    nis                 = fields.Char(string="N.I.S",  compute='elo_compute_fiscal_information', inverse='elo_inverse_nis',help="Numéro d'identification statistique")
    nif                 = fields.Char(string="N.I.F", compute='elo_compute_fiscal_information', inverse='elo_inverse_nif', help="Numéro d'identification fiscale")
    fax                 = fields.Char(string="Fax", compute='elo_compute_fiscal_information', inverse='elo_inverse_fax',  size=64)
    capital_social      = fields.Float(string="Capital Social", digits=dp.get_precision('Account'))

    def elo_compute_fiscal_information(self):
