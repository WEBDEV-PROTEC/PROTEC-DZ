...
class WebsiteSaleExt(WebsiteSale):
    ...
    def get_attribute_value_count(self, products, is_brand=False):
        ...
        from_clause = from_clause.strip('"').strip("'")
        ...
        if is_brand:
            ...
            query = sql.SQL("""select product_brand_id,count(id) as count 
                        from product_template 
                        where id in (SELECT product_template.id FROM {from_clause} {where_str}) 
                        group by product_brand_id""").format(
                from_clause=sql.Identifier(from_clause),
            ...
            query = sql.SQL(
                """select product_attribute_value_id,count(DISTINCT product_tmpl_id) as count from product_template_attribute_value where ptav_active = true AND product_tmpl_id in (SELECT product_template.id FROM {from_clause} {where_str}) group by product_attribute_value_id""").format(
                from_clause=sql.Identifier(from_clause),
                where_str=sql.SQL(where_str)
            )
            request._cr.execute(query, where_clause_params)
