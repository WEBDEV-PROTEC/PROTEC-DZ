...
publicWidget.registry.productsSearchBar.include({
    ...
    _fetch: function () {
        var options = {'order': this.order, 'limit': this.limit, 'display_price':false,'display_description': this.displayDescription, 'max_nb_chars': Math.round(Math.max(this.autocompleteMinWidth, parseInt(this.$el.width())) * 0.22)};
        if (this.categoryID) {
