...
class ThemePrimeWebsiteSale(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        if theme_id and theme_id.name.startswith('theme_prime'):
            ...
            lol_domain = self._get_search_domain(search, category, attrib_values)
            Product = request.env['product.template'].with_context(bin_size=True)
            search_product = Product.search(lol_domain, order=False)
            brand_ids_lol = [elt.id for elt in search_product]
            
            available_value_ids = []
            for elt in search_product: 
                line_ids_lol  = elt.attribute_line_ids
                for elt in line_ids_lol :
                    value_ids_lol = elt.value_ids
                    for elt in value_ids_lol:
                        available_value_ids.append(elt.name)
            available_value_ids = list(dict.fromkeys(available_value_ids))
            
            
            brands_domain =  expression.AND([request.website.website_domain(), [('product_ids', 'in', brand_ids_lol)]])
        
            brands = request.env['dr.product.brand'].search(brands_domain)
